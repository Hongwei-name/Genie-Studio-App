...
class StatsPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final buckets = _activityBuckets(logs.entries);
          ...
          LayoutBuilder(
            builder: (context, constraints) {
              ...
              final activity = _ActivityPanel(
                buckets: buckets,
                eventCount: logs.entries.length,
              );
  ...
  List<int> _activityBuckets(List<LogEntry> entries) {
    final now = DateTime.now();
    final values = List<int>.filled(8, 0);
    for (final entry in entries) {
      final delta = now.difference(entry.time).inHours;
      if (delta >= 0 && delta < 8) values[7 - delta]++;
    }
    return values;
  }
}
...
class _ActivityPanel extends StatelessWidget {
  const _ActivityPanel({required this.buckets, required this.eventCount});
      ...
      child: SizedBox(
        ...
        child: Row(
          ...
          children: List.generate(buckets.length, (index) {
            final height = 12 + (buckets[index] / maxValue) * 86;
            return Expanded(
              child: Padding(
